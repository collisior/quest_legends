...
public class DisplayBoard implements Color, CellType {
	...
	public static void showQuestBoard(Quest quest, QuestBoard board) {
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) { // monster side
				...
				Monster monster = monsterIsHere(i, j, quest);
				if (monster != null) {
					tmp = getMonsterLine(board, i, j, monster);
				} else {
					tmp = getMiddleLine(board, i, j);
				}
				b += tmp;
				tmp = "";
			}
			...
			for (int j = 0; j < cols; j++) { // hero side
				...
				Player player = playerIsHere(i, j, quest);
				if (player != null) {
					tmp = getPlayerLine(board, i, j, player);
				} else {
					tmp = getMiddleLine(board, i, j);
				}
				b += tmp;
	 ...
	 */
	private static Player playerIsHere(int row, int col, Quest quest) {
	 ...
	 */
	private static Monster monsterIsHere(int row, int col, Quest quest) {
	...
	private static String getPlayerLine(QuestBoard board, int i, int j, Player player) {
		String tmp = "";
	...
	private static String getMiddleLine(QuestBoard board, int i, int j) {
		String tmp = "";
		tmp = ll + BLACK + " ,`  `" + RESET; // Plain
		if (board.board[i][j].getType() == NEXUS) { // Magic MArket
